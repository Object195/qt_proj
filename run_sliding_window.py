...
def run_sliding_window(
    ...
    print(f"  Gap          : {gap}")
    print(f"  Filter Iters : {n_filters}")
    ...
    for filter_iter in range(n_filters + 1):
        ...
        print(f"Using processor: {current_processor_file}")
        ...
        with tqdm(total=total_iters, desc=f"Progress (Series {filter_iter+1})") as pbar:
            while True:
                ...
                create_model_data.run(
                    ...
                    output_dir=data_dir,
                    verbose=verbose,
                    processor_file=current_processor_file
                )
        ...
        print(f"\n=== SLIDING WINDOW LOOP COMPLETE (Series {filter_iter + 1}) ===")
        
        if filter_iter < n_filters:
            print(f"\n=== RUNNING SHAP FILTER {filter_iter + 1}/{n_filters} ===")
            feature_filter_shap.run_shap_filter(
                processor_file=current_processor_file,
                output_processor_file=output_processor_file
            )
            current_processor_file = output_processor_file
            
    print(f"\nAll {n_filters + 1} sliding window runs completed.")
    print(f"All models and datasets for the final run saved in: {base_dir}")
